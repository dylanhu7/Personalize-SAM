...
from show import show_mask, show_points
import argparse
import os
import warnings
...
def get_arguments():
    ...
    parser.add_argument(
        "--ckpt", type=str, default="./segment-anything/sam_vit_h_4b8939.pth"
    )

    parser.add_argument("--ref_idx", type=int, default=0)
    parser.add_argument("--idx_format", type=str, default="%02d")
...
def main():
    ...
    # obj_name is last component of path
    obj_name = os.path.basename(args.data)
...
def persam(args, obj_name, images_path, masks_path, output_path):
    ...
    # Path preparation
    ref_idx = args.idx_format % args.ref_idx
    # ref_image_path = os.path.join(images_path, obj_name, ref_idx + "." + args.image_ext)
    # ref_mask_path = os.path.join(masks_path, obj_name, ref_idx + ".png")
    # test_images_path = os.path.join(images_path, obj_name)
    ref_image_path = os.path.join(images_path, ref_idx + "." + args.image_ext)
    ref_mask_path = os.path.join(masks_path, ref_idx + ".png")
    ...
    for test_idx in tqdm(range(len(os.listdir(test_images_path)))):
        # Load test image
        test_idx = args.idx_format % test_idx
        test_image_path = test_images_path + "/" + test_idx + "." + args.image_ext
